...
public class ProdutoDAO {
	...
	public List<Produto> buscarPorParametroComCriteria(String nome, BigDecimal preco, LocalDate dataCadastro) {
		...
		Predicate filtros = criteria.and();
		
		if(nome != null && !nome.trim().isEmpty()) {
			filtros = criteria.and(filtros, criteria.equal(from.get("nome"), nome));
		}
		if(preco != null) {
			filtros = criteria.and(filtros, criteria.equal(from.get("preco"), preco));
		}
		if(dataCadastro != null) {
			filtros = criteria.and(filtros, criteria.equal(from.get("dataCadastro"), dataCadastro));
		}
